    ...
    if (food.length !== 0) {
      ...
      for (var i = 0; i < food.length; i++) {
        let cartTxt =
          ...
          '<p></div>$ ' +
          food[i][2] +
          '</i></p>  </div>  <div class="col-2"> <p class="text-muted-small" > Price ' +
          food[i][1] * food[i][2] +
...
async function makeOrder() {
  ...
  // body should be { items: food } and address
  // if successful, show a success message
  // if not, show an error message
  const address = document.getElementById("address").value;
  const items = food.map(([name, quantity, price]) => ({ name, quantity, price }));
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert("Failed to place order: " + errorData.message); // Display the error message
